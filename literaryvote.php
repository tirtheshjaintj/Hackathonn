...
<head>
   ...
   <link rel="stylesheet" href="register.css">
    <title>Pcte literary Club Voting</title>
   
    <style>
      body{
        background:url(clubs/L.jpg);
...
<center><h1>Vote For Literary Club  📝</h1></center>
  <form action="literaryvote.php" method="post">
  <div class="mb-3">
...
$query="SELECT * FROM literary WHERE rollno =$rollno";
...
if(mysqli_num_rows(mysqli_query($con,$query))>0){
  ...
  while($row=mysqli_fetch_assoc($result)){
    ...
    if(password_verify($pass,$row['password'])){
      if($row["status"]=="0"){
     $query1="UPDATE `literary` SET `status`='1' WHERE rollno=$rollno";
     $con->query($query1);
     
     $query2="UPDATE literaryvote SET vote = vote + 1 WHERE name='$president' and post='president'";
     $con->query($query2);
      $query3="UPDATE literaryvote SET vote = vote + 1 WHERE name='$vicepresident' and post='vicepresident'";
     $con->query($query3);
      $query4="UPDATE literaryvote SET vote = vote + 1 WHERE name='$secretary' and post='secretary'";
     $con->query($query4);
