...
<head>
   ...
   <link rel="stylesheet" href="register.css">
    <title>Pcte Sports Club Voting</title>
  
    <style>
      body{
        background:url(clubs/r2.jpg);
...
<center><h1>Vote For Sports Club 🏅</h1></center>
  <form action="sportsvote.php" method="post">
 ...
 <option></option>
  ...
  <div class="mb-3">
    <label for="secretary" class="form-label">Select Secretary</label>
...
$query="SELECT * FROM sports WHERE rollno =$rollno";
...
if(mysqli_num_rows(mysqli_query($con,$query))>0){
  ...
  while($row=mysqli_fetch_assoc($result)){
    ...
    if(password_verify($pass,$row['password'])){
      if($row["status"]=="0"){
     $query1="UPDATE `sports` SET `status`='1' WHERE rollno=$rollno";
     $con->query($query1);
     
     $query2="UPDATE sportsvote SET vote = vote + 1 WHERE name='$president' and post='president'";
     $con->query($query2);
      $query3="UPDATE sportsvote SET vote = vote + 1 WHERE name='$vicepresident' and post='vicepresident'";
     $con->query($query3);
      $query4="UPDATE sportsvote SET vote = vote + 1 WHERE name='$secretary' and post='secretary'";
     $con->query($query4);
